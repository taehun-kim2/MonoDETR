...
from lib.datasets.custom.custom_eval_python.eval import get_official_eval_result
from lib.datasets.custom.custom_eval_python.eval import get_distance_eval_result
import lib.datasets.custom.custom_eval_python.custom_common as custom
import copy

from lib.datasets.custom.pd import PhotometricDistort
from lib.datasets.custom.custom_config import class_name, class_mean_size, class_conversion_dict
...
class Custom_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        ...
        self.filenames = [x.strip() for x in open(self.split_file).readlines()]
        ...
        self.cls_mean_size = np.array(class_mean_size)
    ...
    def get_image(self, idx=None, filename=None):
        assert (idx is None) ^ (filename is None)
        if idx is not None:
            target = self.filenames[idx]
        elif filename is not None:
            target = filename
        img_file = os.path.join(self.image_dir, target + '.jpg')
        assert os.path.exists(img_file)
        return Image.open(img_file)    # (H, W, 3) RGB mode

    def get_label(self, idx=None, filename=None):
        assert (idx is None) ^ (filename is None)
        if idx is not None:
            target = self.filenames[idx]
        elif filename is not None:
            target = filename
        label_file = os.path.join(self.label_dir, target + '.txt')
        assert os.path.exists(label_file)
        objects = get_objects_from_label(label_file)
        for i, object in enumerate(objects):
            objects[i].cls_type = class_conversion_dict[object.cls_type]
        return objects

    def get_calib(self, idx=None, filename=None):
        assert (idx is None) ^ (filename is None)
        if idx is not None:
            target = self.filenames[idx]
        elif filename is not None:
            target = filename
        calib_file = os.path.join(self.calib_dir, target + '.txt')
        assert os.path.exists(calib_file)
        return Calibration(calib_file)

    def eval(self, results_dir, logger):
        logger.info("==> Loading detections and GTs...")
        filenames = [i + '.txt' for i in self.filenames]
        dt_annos = custom.get_label_annos(results_dir)
    ...
    def __getitem__(self, index):
        ...
        info = {'img_id': self.filenames[index],
                'img_size': img_size,
...
if __name__ == '__main__':
    ...
    cfg = {'root_dir': 'data/monodetr-sample',
    ...
    for batch_idx, (inputs, calib, targets, info) in enumerate(dataloader):
        ...
        print(targets['size_3d'][0][0], targets['labels'])
