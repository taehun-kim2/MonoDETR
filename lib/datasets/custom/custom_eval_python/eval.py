...
import numba
import io as sysio
from lib.datasets.custom.custom_eval_python.rotate_iou import rotate_iou_gpu_eval

from lib.datasets.custom.custom_config import class_name
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    overlap = np.zeros((3, len(class_name)))
    overlap[0, :] = 0.5  # 2D BBox
    overlap[1, :] = 0.25  # 3D BBox
    overlap[2, :] = 0.25  # BEV BBox
    
    min_overlaps = np.stack([overlap], axis=0)  # [2, 3, 5]
    
    class_to_name = dict(zip([i for i in range(len(class_name))], class_name))
